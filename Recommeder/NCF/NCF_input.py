...
DATA_DIR = 'D:\projects\ML_learning\Recommeder\data\\ratings.dat'
DATA_PATH = 'D:\projects\ML_learning\Recommeder\data\\'
...
def load_data():
    full_data = pd.read_csv(DATA_DIR,sep="::",header=None,names=COLUMN_NAMES,usecols=[0,1],
                            dtype={0:np.int,1:np.int},engine='python')
    full_data.user = full_data['user']-1 #广播,将所有user_id-1，从0开始
    ...
    item_map = re_index(item_set)  # item_id不是连续的,所以re_index
    item_list = []
    ...
    for i in range(len(full_data)):
        u = full_data['user'][i]
        t = full_data['item'][i]
        if u not in user_rating:
            user_rating[u]=[]
        user_rating[u].append(t)

    user_negative = {}
    for key in user_rating:
        user_negative[key] = list(item_set - set(user_rating[key]))
