        ...
        with self.graph.as_default():
            ...
            for i in range(self.deep_init_size):
                linear_output.append(
                    tf.reshape(tf.reduce_sum(tf.multiply(self.embeddings,self.weights[['product-linear'][i]]),axis=[1,2]),
                               shape=(-1,1))) #field_size*1
            ...
            if self.use_inner:
                ...
                # expand_dims在第n维添加1
                p = tf.matmul(tf.expand_dims(embedding_sum,2),tf.expand_dims(embedding_sum,1)) #Ｎ＊Ｋ＊Ｋ
                for i in range(self.deep_init_size):
            ...
            self.y_deep = tf.nn.relu(tf.add(tf.add(tf.lz,tf.lp,),self.weights['product-bias']))
