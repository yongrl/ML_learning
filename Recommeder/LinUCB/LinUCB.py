...
class LinUCB:
    ...
    def update(self,reward):
        if reward == -1:
            ...
            self.AaI[self.a_ma] = np.linalg.inv(self.Aa[self.a_max]) # inv矩阵求逆
    ...
    def recommend(self,timestamp,user_features,articles):
        ...
        art_max = articles[np.argmax(np.dot(xaT,theta_tmp) +
                    self.alpha * np.sqrt(np.dot(np.dat(xaT,AaI_tmp),xa)))]
